...
class StepByStep:
  def __init__(self, model, loss_fn, optimizer):
    # Here we define the attr of our class
    self.model = model
    self.loss_fn = loss_fn
    self.optimizer = optimizer
    self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
    # Send the model to the specified deviced right away
    self.model.to(self.device)

    # These attr are defined here, but since thy are not
    # available at the moment of creation, we keep them None
    ...
    # These attr are going to be computed internally
    self.losses = []
    self.val_losses = []
    self.learning_rates = []
    self.total_epochs = 0
    ...
    # Create the train_step function for our model,
  ...
  def to(self, device):
    ...
    # Its sets the corresponding attr (to be used later in the mini-batches)
    ...
    try:
      ...
      self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
      print(f"Could not sent it to {device}, sending it to {self.device} instead")
      self.model.to(self.device)

  def set_loaders(self, train_loader, val_loader=None):
    # This method allows the user the define which train_loader
    # (and val_loader, optionallay) to use
    # Both loaders are then assigned to attr of the class
    # So they can be referred to later
    self.train_loader = train_loader
    self.val_loader = val_loader

  def set_tensorboard(self, name, folder='runs'):
    # This method allows the user to create SummaryWriter to
  ...
  def _make_train_step_fn(self):
    ...
    # the attr: self.model, self.loss, self.optimizer

    # Builds function that perform a step in the train loop
    def perform_train_step_fn(x, y):
      self.model.train()

      yhat = self.model(x)
      if yhat.shape != y.shape:
        yhat = yhat.reshape(y.shape).type(torch.float32)
  ...
  def _mini_batch(self, validation=False):
    # The mini-batch can be used with boath loaders
    if validation:
    ...
    # same mini-batch loop we had befor
    mini_batch_losses = []
    for i, (x, y) in enumerate(data_loader):
      x, y = x.type(torch.float32).to(self.device), y.type(torch.float32).to(self.device)
  ...
  def train(self, n_epochs, seed=42):
    ...
    for epoch in range(n_epochs):
      ...
      with torch.inference_mode():
        # Perform eval using mini-batches
  ...
  def predict(self, x):
    self.model.eval()
    # Takes numpy input and maek it a float tensor
    x_tensor = torch.as_tensor(x).float()
    y_hat_tensor = self.model(x_tensor.to(self.device))
    # Set it back to train mode
    self.model.train()
    # Detach it, brings it to CPU and back to numpu
  ...
  def add_graph(self):
    if self.train_loader and self.writer:
      # Fetches single mini-batches so we can use add_graph
  ...
  def loader_apply(loader, func, reduce='sum'):
    result = [func(x, y) for i, (x, y) in enumerate(loader)]
    result = torch.stack(result, axis=0)

    if reduce == 'sum':
      result = result.sum(axis=0)
    elif reduce == 'mean':
      result = result.float().mean(axis=0)

    return result

  @staticmethod
  def statistic_per_channel(images, labels):
    # NCHW
    n_samples, n_channels, n_height, n_weight = images.size()
    # Flatten HW in to a single dimension
    flatten_per_channel = images.reshape(n_samples, n_channels, -1)

    # Computes statistic o each image per channel
    # Average pixel value per channel
    # (n_samples, n_channels)
    means = flatten_per_channel.mean(axis=2)
    # Standard deviation of pixel values per channels
    # (n_samples, n_channels)
    stds = flatten_per_channel.std(axis=2)

    # Adds up statistic of all images in a mini-batch
    # (1, n_channels)
    sum_means = means.sum(axis=0)
    sum_stds = stds.sum(axis=0)

    # Makes a tensor of shape (1, n_channelabss)
    # with the numebr of samples in hte mini-batch
  ...
  def make_normalizer(loader):
    total_samples, total_means, total_stds = StepByStep.loader_apply(
      loader, StepByStep.statistic_per_channel
    )
